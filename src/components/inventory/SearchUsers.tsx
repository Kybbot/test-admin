...
interface Props {
  createBond: (id: number) => void;
  hideSearch: HandleToggle;
  users: User[];
}

const SearchUsers: React.FC<Props> = ({ hideSearch, users, createBond }) => {
  ...
  const searchItem = search(users, searchValue, ({ name }) => name);
        ...
        {(!searchItem.length ? (
          ...
          <>
            {searchItem.map((user: User) => (
              <ItemWrap key={user._id} onClick={() => createBond(user._id)}>
                <ItemInfo>
                  <LinkUser>
